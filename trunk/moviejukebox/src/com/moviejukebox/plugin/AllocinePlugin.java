...
public class AllocinePlugin extends ImdbPlugin
{
  ...
  protected void updateTVShowInfo( Movie movie )
  {
    try
    {
      ...
      for ( String acteur : extractTags( xml, "<h4>Avec", "</h4>",
          "personne/fichepersonne_gen_cpersonne", "</a>" ) )
      {
    ...
    {
      logger.severe( "Failed retreiving alloCine TVShow info for "
  ...
  public void scanTVShowTitles( Movie movie )
  {
    ...
    if ( !movie.isTVShow() || !movie.hasNewMovieFiles() || allocineId == null
        || allocineId.equalsIgnoreCase( Movie.UNKNOWN ) )
    {
      return;
    }
          ...
          "<a href=\"/series/episodes_gen_csaison", "</a>" ) )
      {
        try
        {
          ...
          if ( seasonId == movie.getSeason() )
          {
            ...
            xml = webBrowser.request( "http://www.allocine.fr/series/episodes_gen_csaison="
                + seasonAllocineId + "&cserie=" + allocineId + ".html" );
  ...
  private boolean updateMovieInfo( Movie movie )
  {
    try
    {
      ...
      int count = 0;
      for ( String genre : extractTags( xml, "<h4>Genre : ", "</h4>",
          "film/alaffiche_genre_gen_genre", "</a>" ) )
      {
    ...
    {
      logger.severe( "Failed retreiving allocine infos for movie : "
          + movie.getId( ALLOCINE_PLUGIN_ID ) );
      e.printStackTrace();
    }
    return true;
  ...
  private void updatePoster( Movie movie )
  {
    ...
    try
    {
      ...
      {
        posterURL = "http://posters.motechnet.com/covers/" + movie.getId( IMDB_PLUGIN_ID )
            + "_largeCover.jpg";
        logger.finest( "Movie PosterURL : " + posterURL );
        movie.setPosterURL( posterURL );
        return;
      } // Check www.impawards.com
      else if ( !( posterURL = this.testImpawardsPoster( movie.getId( IMDB_PLUGIN_ID ) ) )
  ...
  /**
   * retrieve the allocineId matching the specified movie name. This routine is base on a alloCine
   * search.
   * 
   * @throws ParseException
   */
  private String getAllocineId( String movieName, String year, Movie mediaFile )
      throws ParseException
  {
    ...
    try
    {
      ...
      for ( String searchResult : extractTags( xml, alloCineStartResult,
          "<script type=\"text/javascript\">", alloCineMediaPrefix, "</h4></div>" ) )
      {
        // logger.finest("AlloCine SearchResult = " + searchResult);
        String searchResultYear = searchResult.substring( searchResult.lastIndexOf( ">" ) + 1,
            searchResult.length() );
        // logger.finest("AlloCine searchResultYear = " + searchResultYear);
        if ( year == null || year.equalsIgnoreCase( Movie.UNKNOWN )
            || year.equalsIgnoreCase( searchResultYear ) )
        {
      ...
      String startTag, String endTag )
  {
    ...
    while ( index != -1 )
    {
      ...
      tags.add( HTMLTools.decodeHtml( text.trim() ).replaceAll(
          "^[\\s\\p{Zs}\\p{Zl}\\p{Zp}]*\\b(.*)\\b[\\s\\p{Zs}\\p{Zl}\\p{Zp}]*$", "$1" ) );
      endIndex += endLen;
  ...
  protected ArrayList< String > extractHtmlTags( String src, String sectionStart,
