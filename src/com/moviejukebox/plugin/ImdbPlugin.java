...
public class ImdbPlugin implements MovieDatabasePlugin
{
  ...
  public String testImpawardsPoster( String movieId )
  {
    ...
    {
      content = webBrowser
          .request( "http://search.yahoo.com/search?fr=yfp-t-501&ei=UTF-8&rd=r1&p=site:impawards.com+link:http://www.imdb.com/title/"
              + movieId );
    }
    catch ( Exception e )
    {}
    ...
    if ( content != null )
    {
      ...
      if ( indexMovieLink != -1 )
      {
        ...
        String imgRepUrl = "http://www.impawards.com/" + finMovieUrl.substring( 0, indexLastRep )
            + "/posters";
        returnString = imgRepUrl
            + finMovieUrl.substring( indexLastRep, finMovieUrl.lastIndexOf( '.' ) ) + ".jpg";
      }
    }
      ...
      if ( content != null )
      {
        ...
        if ( indexMovieLink != -1 )
        {
          ...
          returnString = "http://www.moviecovers.com/getjpg.html/"
              + finMovieUrl.substring( 0, finMovieUrl.lastIndexOf( '.' ) ).replace( "+", "%20" )
              + ".jpg";
        }
      }
  ...
  {
    String imdbId = movie.getId( IMDB_PLUGIN_ID );
    if ( !movie.isTVShow() || !movie.hasNewMovieFiles() || imdbId == null
        || imdbId.equalsIgnoreCase( Movie.UNKNOWN ) )
    {
      return;
    }
    ...
    try
    {
      ...
      for ( MovieFile file : movie.getMovieFiles() )
      {
        ...
        String episodeName = HTMLTools.extractTag( xml, "Season " + season + ", Episode " + episode
            + ":", 2 );
  ...
  private String getLongPlot( Movie movie )
  {
    ...
    try
    {
      ...
      if ( !result.equalsIgnoreCase( Movie.UNKNOWN )
          && result.indexOf( "This plot synopsis is empty" ) < 0 )
      {
        plot = result;
      }
    }
