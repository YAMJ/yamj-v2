 ...
 *
 * It is possible to specify URL of the movie page on kinopoisk in the .nfo file. In this case movie data will be retrieved from
 * this page only.
 */
public class KinopoiskPlugin extends ImdbPlugin {
    ...
    private final boolean scrapeWonAwards = "won".equalsIgnoreCase(tmpAwards);
    ...
    private final boolean companyAll = "all".equalsIgnoreCase(PropertiesUtil.getProperty("kinopoisk.company", "first"));
    private final boolean countryAll = "all".equalsIgnoreCase(PropertiesUtil.getProperty("kinopoisk.country", "first"));
...
//            mediaFile.setTitle(Movie.UNKNOWN);
    ...
    public String getKinopoiskId(String movieName, String year, int season) {
        try {
            ...
            sb = "https://kinopoisk.ru/index.php?level=7&from=forma&result=adv&m_act[from]=forma&m_act[what]=content" + sb;
            ...
            int beginIndex = xml.indexOf("data-id=\"");
            if (beginIndex == -1) {
                ...
                // It's the movie page
                StringTokenizer st = new StringTokenizer(xml.substring(beginIndex + 9), "\"");
                kinopoiskId = st.nextToken();
            }
    ...
    private boolean updateKinopoiskMediaInfo(Movie movie, String kinopoiskId) {
        try {
            ...
            for (String item : HTMLTools.extractTags(xml, "<table class=\"info\">", "</table>", "<tr", "</tr>")) {
                ...
                if (OverrideTools.checkOverwriteGenres(movie, KINOPOISK_PLUGIN_ID)) {
                    ...
                    for (String genre : HTMLTools.extractTags(item, ">жанр<", "</tr>", "<a href=\"/lists/m_act%5Bgenre%5D/", "</a>")) {
                        ...
                        if ("мультфильм".equalsIgnoreCase(genre)) {
                ...
                // Certification from Russia or MPAA
            ...
            if (!nfoRating) {
                valueFounded = false;
                for (String rating : HTMLTools.extractTags(xml, kinopoiskId + "/votes/\"", "</a>", "<span", "</span>")) {
                ...
                if (poster || etalonFlag) {
                    ...
                    if (StringTools.isValidString(wholeArts)) {
                        if (wholeArts.contains("<table class=\"fotos")) {
                                String picture = HTMLTools.extractTag(wholeArts, "src=\"https://st.kp.yandex.net/images/poster/sm_", 0, "\"");
                            if (StringTools.isValidString(picture)) {
                                posterURL = "https://st.kp.yandex.net/images/poster/" + picture;
                                valueFounded = true;
                            }
                        }
                    }
            ...
            if (StringTools.isNotValidString(fanURL) || (!nfoFanart && (fanArt || kadr)) || etalonFlag || fanURL.contains("originalnull")) {
                ...
                try {
                    ...
                    String wholeArts = httpClient.request(FILM_URL + kinopoiskId + "/wall/", CHARSET);
                    if (StringTools.isValidString(wholeArts)) {
                        if (wholeArts.contains("<table class=\"fotos")) {
                            String picture = HTMLTools.extractTag(wholeArts, "src=\"https://st.kp.yandex.net/images/wallpaper/sm_", 0, ".jpg");
                            if (StringTools.isValidString(picture)) {
                                ...
                                wholeArts = httpClient.request("https://www.kinopoisk.ru/picture/" + picture + "/w_size/" + size, CHARSET);
                                if (StringTools.isValidString(wholeArts)) {
                                    picture = HTMLTools.extractTag(wholeArts, "src=\"//st.kp.yandex.net/im/wallpaper/", 0, "\"");
                                    if (StringTools.isValidString(picture)) {
                                        fanURL = "https://st.kp.yandex.net//im/wallpaper/" + picture;
                                        valueFounded = true;
                                    }
                                }
                            }
                    ...
                    if (kadr && (StringTools.isNotValidString(fanURL) || fanURL.contains("originalnull"))) {
                        ...
                        if (StringTools.isValidString(wholeArts)) {
                            ...
                            if (photosInd != -1) {
                                String picture = HTMLTools.extractTag(wholeArts, "src=\"https://st.kp.yandex.net/images/kadr/sm_", 0, "\"");
                                LOG.debug("Добивка для Fanart(кадры):" + picture);
                                if (StringTools.isValidString(picture)) {
                                    fanURL = "https://st.kp.yandex.net//images/kadr/" + picture;
                                    LOG.debug("Ссылка на Fanart(кадры):" + fanURL);
                                    valueFounded = true;
                                }
                            }
                        }
            ...
            if ((scrapeAwards && !nfoAwards) || etalonFlag) {
                ...
                xml = httpClient.request(FILM_URL + kinopoiskId + "/awards/", CHARSET);
                ...
                if (StringTools.isValidString(xml)) {
                    ...
                    for (String tmp : HTMLTools.extractTags(xml, ">Кассовые сборы<", "</table>", "<tr><td colspan=2", "</h3>")) {
                        ...
                        if (StringTools.isValidString(country)) {
                            ...
                            if (!"--".equals(money)) {
                                movie.setGross("В России".equals(country) ? "Russia" : "В США".equals(country) ? "USA"
                                        : "Общие сборы".equals(country) ? "Worldwide" : "В других странах".equals(country) ? "Others" : country,
                                        money);
                                valueFounded = true;
                            }
                        }
    ...
    private boolean updateKinopoiskPersonInfo(Person person) {
        ...
        try {
            String xml = httpClient.request(NAME_URL + kinopoiskId + "/view_info/ok/", CHARSET);
            if (StringTools.isValidString(xml)) {
                ...
                if (xml.contains(">место рождения</td><td>")) {
                    ...
                    if (StringTools.isValidString(bp)) {
                        ...
                        if (StringTools.isValidString(bp) && !"-".equals(bp)) {
                ...
                if (xml.contains("https://st.kinopoisk.ru/images/actor/" + kinopoiskId + ".jpg") && StringTools.isNotValidString(person.getPhotoURL())) {
                    person.setPhotoURL("https://st.kinopoisk.ru/images/actor/" + kinopoiskId + ".jpg");
                }
                ...
                if (xml.contains("/film/" + kinopoiskId + "/cast/") && StringTools.isNotValidString(person.getBackdropURL())) {
                    ...
                    if (StringTools.isValidString(xml)) {
                        ...
                        if (StringTools.isValidString(size)) {
                            ...
                            xml = httpClient.request("https://www.kinopoisk.ru/picture/" + id + "/w_size/" + size + "/", CHARSET);
                            if (StringTools.isValidString(xml)) {
                                int beginInx = xml.indexOf("https://st.kinopoisk.ru/im/wallpaper");
                                if (beginInx > -1) {
            ...
            if (!"imdb".equals(preferredRating)) {
                xml = httpClient.request(NAME_URL + kinopoiskId + "/sort/rating/", CHARSET);
                if (StringTools.isValidString(xml)) {
                    ...
                    for (String block : HTMLTools.extractTags(xml, "<center><div style='position: relative' ></div></center>", "<tr><td><br /><br /><br /><br /><br /><br /></td></tr>", "<tr><td colspan=3 height=4><spacer type=block height=4></td></tr>", "</div>        </td></tr>")) {
                        ...
                        if (Filmography.JOB_PRODUCER.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_PRODUCER);
                        } else if (Filmography.JOB_DIRECTOR.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_DIRECTOR);
                        } else if (Filmography.JOB_WRITER.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_WRITER);
                        } else if (Filmography.JOB_ACTOR.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_ACTOR);
                        } else if (Filmography.JOB_EDITOR.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_EDITOR);
                        } else if ("himself".equals(job) || "herself".equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_THEMSELVES);
                        } else if (Filmography.JOB_DESIGN.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_DESIGN);
                        } else if (Filmography.JOB_OPERATOR.equals(job)) {
                            StringUtils.capitalize(Filmography.JOB_OPERATOR);
                        } else if (Filmography.JOB_COMPOSER.equals(job)) {
                        ...
                        for (String item : HTMLTools.extractTags(block + "endmarker", "<tr><td colspan=3 style=\"padding-left:20px\">", "endmarker", "<div class=\"item\">", "</div></div>")) {
                            ...
                            String url = "https://www.kinopoisk.ru/level/1/film/" + id + "/";
                    ...
                    if (!newFilmography.isEmpty()) {
                            ...
                            for (Filmography film : person.getFilmography()) {
                                ...
                                for (Filmography f : newFilmography.values()) {
                                    ...
                                    if (name.equalsIgnoreCase(name2) || name2.equalsIgnoreCase(title) || name2.equalsIgnoreCase(originalTitle)
                                        ...
                                        film.setDepartment();
                                        film.setRating(Integer.toString((int) ("combine".equals(preferredRating)
                        ...
                        } else if ("kinopoisk".equals(preferredRating) || (person.getFilmography().isEmpty())) {
    ...
    private String getKinopoiskPersonId(String person, String mode) {
        ...
        try {
            ...
            for (int step = 0; step < 3 && StringTools.isNotValidString(personId); step++) {
                String xml = httpClient.request("https://www.kinopoisk.ru/index.php?level=7&from=forma&result=adv&m_act[from]=forma" + (step < 2 ? ("&m_act[what]=actor") : "") + sb + (step == 0 ? ("&m_act[work]=" + mode) : ""), CHARSET);
