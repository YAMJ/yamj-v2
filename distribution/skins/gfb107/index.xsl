...
<xsl:for-each select="library/category[@current='true']/index[@current='true']">
  <td width="*">
    <a class="heading" tvid="HOME">
      <xsl:attribute name="href"><xsl:value-of select="$mjb.homePage" />.html</xsl:attribute><xsl:value-of select="../@name" />: <xsl:value-of select="@name" /></a></td>
...
<table class="movies" border="0" cellpadding="0" cellspacing="0">
